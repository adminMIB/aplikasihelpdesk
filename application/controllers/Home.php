...
function index()
    {
    	if($this->session->userdata('is_customer')===true)
    	{
	        ...
	        $data['body'] = "body/dashboard_customer";
	        $sql_ticket_solved = "SELECT COUNT(id_ticket) AS jml_ticket_solved FROM ticket where status = 6 and reported = '".$this->session->userdata('id_user')."'";
			$row_ticket_solved = $this->db->query($sql_ticket_solved)->row()->jml_ticket_solved;

			$sql_ticket_process = "SELECT COUNT(id_ticket) AS jml_ticket_process FROM ticket where status = 4 and reported = '".$this->session->userdata('id_user')."'";
			$row_ticket_process = $this->db->query($sql_ticket_process)->row()->jml_ticket_process;

			$sql_ticket_app_tek = "SELECT COUNT(id_ticket) AS jml_ticket_app_tek FROM ticket where status = 3 and reported = '".$this->session->userdata('id_user')."'";
			$row_ticket_app_tek = $this->db->query($sql_ticket_app_tek)->row()->jml_ticket_app_tek;

			$sql_feedback_positiv = "SELECT COUNT(history_feedback.id_feedback) AS jml_feedback_positiv FROM history_feedback JOIN ticket ON history_feedback.id_ticket =  ticket.id_ticket WHERE feedback =1 AND ticket.reported ='".$this->session->userdata('id_user')."'";
			$row_feedback_positiv = $this->db->query($sql_feedback_positiv)->row()->jml_feedback_positiv;

			$sql_feedback_negativ = "SELECT COUNT(history_feedback.id_feedback) AS jml_feedback_positiv FROM history_feedback JOIN ticket ON history_feedback.id_ticket =  ticket.id_ticket WHERE feedback =0 AND ticket.reported  ='".$this->session->userdata('id_user')."'";
			$row_feedback_negativ = $this->db->query($sql_feedback_negativ)->row()->jml_feedback_positiv;

			$data['row_ticket_solved'] = $row_ticket_solved;
			$data['row_ticket_process'] = $row_ticket_process;
			$data['row_ticket_app_tek'] = $row_ticket_app_tek;
			$data['sql_feedback_positiv'] = $sql_feedback_positiv;
			$data['sql_feedback_negativ'] = $sql_feedback_negativ;
	        ...
	        LEFT JOIN bagian_departemen E ON E.id_bagian_dept = D.id_bagian_dept WHERE E.id_dept = '$id_dept' AND A.status = 1";
