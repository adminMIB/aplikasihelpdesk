 ...
 function myticket_detail($id)
 {
    $data['header'] = "header/header";
        ...
        $data['body'] = "body/progress_teknisi";
         ...
         $sql = "SELECT A.status,A.jenis_urgensi,A.problem_summary,A.problem_detail, A.progress,A.tanggal, A.tanggal_solved, A.tanggal_proses, A.tanggal_solved, F.nama AS nama_teknisi, D.nama, C.id_kategori, A.id_ticket, A.tanggal, B.nama_sub_kategori, C.nama_kategori
                FROM ticket A 
                LEFT JOIN sub_kategori B ON B.id_sub_kategori = A.id_sub_kategori
                LEFT JOIN kategori C ON C.id_kategori = B.id_kategori 
                LEFT JOIN karyawan D ON D.nik = A.reported 
