...
function __construct(){
 ...
 function ticket_detail($id)
 {
        ...
        $sql = "SELECT A.progress,A.jenis_urgensi, A.status,A.user_file,A.problem_summary,A.problem_detail, D.nama, C.id_kategori, A.id_ticket, A.tanggal, B.nama_sub_kategori, C.nama_kategori
                FROM ticket A 
                LEFT JOIN sub_kategori B ON B.id_sub_kategori = A.id_sub_kategori
                LEFT JOIN kategori C ON C.id_kategori = B.id_kategori 
                LEFT JOIN karyawan D ON D.nik = A.reported 
        ...
        //set sudah dbaca oleh teknisi
        if($this->session->userdata('level')=='TEKNISI')
        {
            $this->model_app->set_status_read_by_teknisi($id);
        }
 ...
 function up_progress()
 {
    ...
    //echo $ticket;exit;
    //set status sudah pernah di acionn oleh teknisi
    $this->model_app->set_act_ticket_teknisi($ticket);
    ...
    $data['action'] =  $action;
    $data['jenis_urgensi'] = strtoupper(trim($this->input->post('urgensi')));
    ...
    $tracking['id_user'] = $id_user;
    $tracking['action_teknisi'] = $action;
    ...
    //update kalau tcket sudah direspon
    $this->model_app->set_direspon_teknisi($ticket);
