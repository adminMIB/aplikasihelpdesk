...
function __construct(){
 ...
 function ticket_list()
 {
        if($this->session->userdata('is_customer')===true)
        {
            ...
            $data['body'] = "body/list_ticket_customer";
            $datalist_ticket = $this->model_app->datalist_ticket('customer' ,$this->session->userdata('id_user'));
            ...
            $q=$this->model_app->datalist_ticket();
 ...
 function view_progress_teknisi($id)
 {
        ...
        $data['notif_list_ticket'] = $row_listticket->jml_list_ticket;
        if(!$this->session->userdata('is_customer')){
        ...
        //end notification
         ...
         $sql = "SELECT A.status,A.jenis_urgensi, A.progress, A.tanggal,A.user_file, A.problem_summary, A.problem_detail,  A.tanggal_proses, A.tanggal_solved, F.nama AS nama_teknisi, D.nama, C.id_kategori, A.id_ticket, A.tanggal, B.nama_sub_kategori, C.nama_kategori
                FROM ticket A 
                LEFT JOIN sub_kategori B ON B.id_sub_kategori = A.id_sub_kategori
                LEFT JOIN kategori C ON C.id_kategori = B.id_kategori 
                LEFT JOIN karyawan D ON D.nik = A.reported 
        ...
        $data['foto'] = $row->user_file;
        $data['summary'] = $row->problem_summary;
        $data['detail'] = $row->problem_detail;
        $data['urgensi'] = $row->jenis_urgensi;
        //TRACKING TICKET
        $data_trackingticket = $this->model_app->data_trackingticket($id);
        $data['data_trackingticket'] = $data_trackingticket;

        //set sudh dibaca oleh admin
        if($this->session->userdata('level')=='ADMIN')
        {
            $this->model_app->set_status_read_by_admin($id);
        }
 ...
 public function pdflistticket()
    ...
    public function pdflistticket_customer()
    {
        ...
        ob_start();
            $content = $this->load->view('body/pdflistticket_customer',$data);
            $content = ob_get_clean();      
            $this->load->library('html2pdf');
            try
            {
